...
function billboardEmail(title: string, body: string, type: string) {
  const accent = type === 'success' ? '#22c55e' : type === 'warning' ? '#eab308' : '#3b82f6'
  return `<!DOCTYPE html>
  ...
  <tr><td align="center">
    <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;">
      ...
      <tr><td style="background:#18181b;border:1px solid #27272a;border-radius:16px 16px 0 0;padding:32px;text-align:center;">
        ...
        <h1 style="margin:0;font-size:28px;font-weight:900;color:#ffffff;letter-spacing:-0.5px;">📢 Gym Billboard</h1>
        <p style="margin:8px 0 0;color:#71717a;font-size:14px;">New announcement from your trainer</p>
      </td></tr>
      <!-- Body -->
      <tr><td style="background:#09090b;border-left:1px solid #27272a;border-right:1px solid #27272a;padding:32px;">
        <div style="border-left:4px solid ${accent};padding:16px 20px;background:#18181b;border-radius:0 8px 8px 0;margin-bottom:24px;">
          <h2 style="margin:0 0 8px;font-size:18px;font-weight:700;color:${accent};">${title}</h2>
          <p style="margin:0;color:#a1a1aa;font-size:15px;line-height:1.6;">${body}</p>
        </div>
        <p style="margin:0;color:#52525b;font-size:13px;">Log in to the Vortex app to view more details and your personalized plan.</p>
      </td></tr>
      <!-- Footer -->
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (type === 'billboard') {
      subject = `📢 Vortex Gym Notice: ${payload.title}`
      html = billboardEmail(payload.title, payload.body, payload.noticeType)
    ...
    // Send one email per recipient (or use `bcc` for bulk)
    const sends = to.map(email =>
      resend.emails.send({
        from: 'Vortex Fitness Club <onboarding@resend.dev>',
        to: email,
        subject,
        html,
      })
