    ...
    <div className="p-4 lg:p-8 max-w-7xl mx-auto space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          ...
          <h1 className="text-3xl font-black text-white mt-1">Client Requests</h1>
        </div>
        ...
        {COLUMNS.map(col => (
          <div key={col.key} className="bg-zinc-950 border border-zinc-800 rounded-xl p-4">
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <div className="text-xs text-zinc-500 font-semibold">{col.label}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      {loading ? (
        <div className="flex gap-4 overflow-x-auto pb-2">
          {COLUMNS.map(col => (
            <div key={col.key} className="space-y-3 min-w-[280px] flex-1">
          ...
          {COLUMNS.map(col => (
            <div key={col.key} className="space-y-3 min-w-[280px] flex-1">
              ...
              {grouped[col.key]?.length === 0 ? (
                ...
                <div key={req.id} className={`bg-zinc-950 border rounded-xl p-4 space-y-3 hover:border-zinc-600 transition-all ${
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <a href={`/trainer/workout-builder?member=${req.member_id || ''}`}
                      className="py-2 text-xs font-bold bg-red-600/10 border border-red-800/30 text-red-400 hover:bg-red-600 hover:text-white rounded-lg transition-all text-center">
                      Build Plan →
                    </a>
                  </div>
                </div>
